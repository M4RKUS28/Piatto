import enum
from sqlalchemy import Boolean, Column, Integer, String, Text, DateTime, ForeignKey, Enum, Index, Float, text
...
class Course(Base):
    ...
    costs = Column(Float, default=0., nullable=True)
...
class Chapter(Base):
    ...
    input_costs = Column(Float, default=0., nullable=True)
    output_costs = Column(Float, default=0., nullable=True)
    ...
    __table_args__ = (
        ...
        # PostgreSQL GIN index on a to_tsvector expression for full-text search
        Index(
            'ix_chapter_fulltext_gin',
            text("to_tsvector('english', coalesce(caption,'') || ' ' || coalesce(summary,'') || ' ' || coalesce(content,''))"),
