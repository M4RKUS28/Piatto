...
export default function LoginFailedPage() {
  ...
  const reason = searchParams.get('reason') || t('loginFailed.defaultReason', 'An unknown error occurred during login');
      ...
      <div className="w-full max-w-lg relative z-10 mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-[#F5F5F5]">
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl font-bold text-[#035035] mb-3">{t('loginFailed.title', 'Login Failed')}</h1>
            <p className="text-[#2D2D2D] text-lg">{t('loginFailed.subtitle', "We couldn't sign you in")}</p>
          </div>

          {/* Error Details */}
          <div className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-6">
            <p className="text-sm font-medium text-red-800 mb-1">{t('loginFailed.errorDetails', 'Error Details:')}</p>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {t('loginFailed.tryAgain', 'Try Again')}
            </Link>
            ...
            >
              ...
              {t('loginFailed.goHome', 'Go Home')}
            </Link>
          </div>

          {/* Help Section */}
          <div className="mt-8 pt-6 border-t border-[#F5F5F5] text-center">
            <p className="text-sm text-[#2D2D2D] mb-3">{t('loginFailed.commonIssues', 'Common issues:')}</p>
            <ul className="text-sm text-[#2D2D2D] space-y-2 text-left">
              <li className="flex items-start gap-2">
                <span className="text-[#FF9B7B] mt-1">•</span>
                <span>{t('loginFailed.issue1', 'Check your email and password are correct')}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#FF9B7B] mt-1">•</span>
                <span>{t('loginFailed.issue2', 'Make sure your account is verified')}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#FF9B7B] mt-1">•</span>
                <span>{t('loginFailed.issue3', 'Clear your browser cookies and try again')}</span>
              </li>
            </ul>
            <p className="text-sm text-[#2D2D2D] mt-4">
              {t('loginFailed.stillTrouble', 'Still having trouble?')}{' '}
              <Link to="/contact" className="text-[#FF9B7B] hover:text-[#035035] font-semibold">
                {t('loginFailed.contactSupport', 'Contact Support')}
              </Link>
            </p>
          </div>
        </div>
