...
from ...utils.auth import get_current_active_write_user
from ..schemas.chat import ChatRequest, ChatResponse
from ...services.chat_service import chat_service
from ...db.crud import chapters_crud, usage_crud
...
async def chat_with_agent(
    ...
    current_user: User = Depends(get_current_active_write_user)
) -> StreamingResponse:
    ...
    try:
        with get_db_context() as db:
            max_chat_usage = dynamic_settings.get_int("MAX_CHAT_USAGE")
            total_chats = usage_crud.get_total_chat_usages(db, str(current_user.id))
            if total_chats >= max_chat_usage:
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail={
                    "error": "LIMIT_REACHED",
                    "code": "MAX_CHAT_USAGE_REACHED",
                    "limit": max_chat_usage,
                    "message": "You have reached the maximum number of chat usages you can use."
                })
