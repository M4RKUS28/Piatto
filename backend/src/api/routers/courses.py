"""
Router for managing courses and chapters.
"""
...
from ...db.crud import courses_crud, chapters_crud, usage_crud
from ...services import course_service
from ...services.course_service import verify_course_ownership
from ...services.settings_service import get_role_based_limit, is_unlimited_for_role
...
async def create_course_request(
    ...
    # Check role-based course creation limits
    user_role = current_user.role.value  # Get the string value of the enum
    
    # Check if user has unlimited course creation privileges
    if not is_unlimited_for_role("COURSE_CREATION", user_role):
        max_course_creations = get_role_based_limit("COURSE_CREATION", user_role)
            ...
            if created_course_count >= max_course_creations:
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": f"You have reached the maximum number of courses you can create ({max_course_creations}) for your role ({user_role})."
                    }
                )
    
    # Check role-based present courses limits
    if not is_unlimited_for_role("PRESENT_COURSES", user_role):
        max_present_courses = get_role_based_limit("PRESENT_COURSES", user_role)
            ...
            if current_courses >= max_present_courses:
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": f"You have reached the maximum number of courses you can have present at the same time ({max_present_courses}) for your role ({user_role})."
                    }
                )
...
async def get_user_courses(
    ...
    value = course_service.get_user_courses( db, current_user.id, skip, limit)
    end_time = time.time()
    import logging
    logging.getLogger(__name__).debug("get_user_courses took %s seconds", end_time - beginning_time)
    return value
