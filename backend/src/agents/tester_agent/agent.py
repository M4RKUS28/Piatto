...
class InitialTesterAgent(StructuredAgent):
    def __init__(self, app_name: str, session_service):
        self.model = dynamic_settings.get("TESTER_INITIAL_AGENT_MODEL")
        self.full_instructions =  load_instruction_from_file("tester_agent/instructions.txt") + "\n" + get_full_instructions()
        ...
        tester_agent = LlmAgent(
            ...
            output_schema=Test,
            global_instruction=lambda _: self.full_instructions,
            instruction="""
...
class CodeReviewAgent(StandardAgent):
    def __init__(self, app_name: str, session_service):
        self.model = dynamic_settings.get("TESTER_CODE_REVIEW_AGENT_MODEL")
        self.full_instructions=get_full_instructions(code_review=True)
        # Create the planner agent
        agent = LlmAgent(
...
class TesterAgent(StandardAgent):
    ...
    async def _review_and_correct_question(self, question: Dict[str, Any], user_id: str, state: dict) -> Optional[
        ...
        for i in range(self.iterations):
            ...
            logger.warning(
                "Question failed validation (Attempt %d/%d). Errors: %s",
                i + 1,
                self.iterations,
        ...
        logger.error("Could not fix code for a question after %d iterations. Discarding question.", self.iterations)
        return None
    ...
    async def run(self, user_id: str, state: dict, content: types.Content, debug: bool = False) -> Dict[str, Any]:
        ...
        if not practice_questions:
            return {"success": True, "questions": [], "inputs": [], "outputs": []}
        ...
        logger.info("Starting parallel review for %d questions...", len(tasks))
        corrected_results = await asyncio.gather(*tasks)
        logger.info("Parallel review complete.")
        ...
        return {
            ...
            "outputs": [str(initial_response)],
            "model": self.inital_tester.model,
        }