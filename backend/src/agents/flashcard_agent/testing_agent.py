...
class TestingFlashcardAgent(StandardAgent):
    ...
    def __init__(self, app_name: str, session_service):
        ...
        self.model = dynamic_settings.get("TESTING_FLASHCARD_AGENT_MODEL")
        ...
        async with semaphore:
            try:
                ...
                if response.get("status") != "success":
                    logger.error("Error in agent response: %s", response)
                    return []
                ...
                logger.exception("Error processing chunk %s: %s", chunk_index, e)
                return []
    ...
    def _parse_questions_response(self, response) -> List[dict]:
        ...
        try:
            ...
            logger.exception("Error parsing questions response: %s", e)
            return []
