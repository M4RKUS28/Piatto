        ...
        try:
            ...
            with get_db_context() as db:
                course_db = courses_crud.update_course(
                    ...
                    image_url=image_response['output'],
            ...
            logger.info("[%s] PlannerAgent responded with %d chapters.", task_id, len(response_planner.get('chapters', [])))
            ...
            async def process_chapter(idx: int, topic: dict):
                ...
                with get_db_context() as db:
                    chapter_db = chapters_crud.create_chapter(
                        ...
                        summary=summary,
                        content=response_code['output'] if 'output' in response_code else "() => {<p>Something went wrong</p>}",
                ...
                response_tester = await self.tester_agent.run(
                    user_id=user_id,
                    state=self.state_manager.get_state(user_id=user_id, course_id=course_id),
                    content=self.query_service.get_tester_query(user_id, course_id, idx, response_code["output"], request.language, request.difficulty)
                )
                ...
                input_costs, output_costs =  0., 0.
                for response in all_responses:
                    ic, oc, fc = self.cost_service.estimate_costs(
                    ...
                    output_costs+=oc
                with get_db_context() as db:
                    chapters_crud.update_chapter_costs(db, chapter_db.id, input_costs, output_costs)
                    courses_crud.update_course_costs(db, course_db.id, input_costs + output_costs)
            ...
            # Calculate costs for the course
            all_responses = [
                info_response,
                response_planner,
                image_response,
            ...
            full_costs = 0.
            for response in all_responses:
                _, _, fc = self.cost_service.estimate_costs(
                    model=response['model'],
                    inputs=response['inputs'],
                    outputs=response['outputs'],
                )
                full_costs+=fc
            with get_db_context() as db:
                courses_crud.update_course_costs(db, course_db.id, full_costs)
            ...
            #})
            logger.info("[%s] Sent completion signal.", task_id)

        except Exception as _:
            
            error_message = f"Course creation failed: {str(traceback.format_exc())}"
            logger.exception("[%s] Error during course creation: %s", task_id, error_message)
            ...
            if course_db:
                try:
                    ...
                    logger.info("[%s] Course %s status updated to FAILED due to error.", task_id, course_id)
                except Exception as db_error:
                    logger.exception("[%s] Additionally, failed to update course status to FAILED: %s", task_id, db_error)
            else:
                logger.warning("[%s] No course_db to update status, error occurred before course creation.", task_id)
            #raise e
            ...
            logger.info("[%s] Finished processing create_course background task.", task_id)
