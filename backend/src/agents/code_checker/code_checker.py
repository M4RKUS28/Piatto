        ...
        if eslint_base_dir:
            ...
            for location in possible_locations:
                if (os.path.exists(os.path.join(location, 'package.json')) and
                    ...
                    import logging
                    logging.getLogger(__name__).info("Found node project in %s", location)
                    break
...
"""
    ...
    logger.info("--- Validating code with a linting error ---")
    result_error = validator.validate_jsx(jsx_code_with_error)
    logger.info(json.dumps(result_error, indent=2))

    logger.info("\n--- Validating correct code ---")
    result_correct = validator.validate_jsx(jsx_code_correct)
    logger.info(json.dumps(result_correct, indent=2))
