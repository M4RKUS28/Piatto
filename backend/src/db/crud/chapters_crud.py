...
def get_completed_chapter_count_by_user_id(db: Session, user_id: str) -> int:
    """Get total number of completed chapters for a user across all their courses"""
    return db.query(Chapter).join(Chapter.course).filter(
        and_(Course.user_id == user_id, Chapter.is_completed == True)
    ).count()

def get_chapter_count_by_user_id(db: Session, user_id: str) -> int:
    """Get total number of chapters for a user across all their courses"""
    return db.query(Chapter).join(Chapter.course).filter(Course.user_id == user_id).count()
...
def search_chapters_indexed(db: Session, query: str, user_id: str, limit: int = 10):
    ...
    # PostgreSQL full-text search replacement for MySQL MATCH/AGAINST
    # Uses to_tsvector over the concatenated fields and plainto_tsquery for the query.
    # Note: for best performance create a GIN index on the tsvector expression in Postgres:
    #   CREATE INDEX chapters_fulltext_idx ON chapters USING GIN (
    #     to_tsvector('english', coalesce(caption,'') || ' ' || coalesce(summary,'') || ' ' || coalesce(content,''))
    #   );
    stmt = text("""
        SELECT 
        ...
        AND to_tsvector('english', coalesce(chapters.caption,'') || ' ' || coalesce(chapters.summary,'') || ' ' || coalesce(chapters.content,''))
...
def update_chapter_costs(db: Session, chapter_id: int, input_costs: float, output_costs: float) -> None:
    """This adds the input_costs and output_costs parameter on top of the current chapter costs"""
    chapter = db.query(Chapter).filter(Chapter.id == chapter_id).first()
    chapter.input_costs += input_costs
    chapter.output_costs += output_costs
    db.commit()
    db.refresh(chapter)
